# Minimal PS-only design for RFSoC4x2: exports a PS-only XSA for boot BSP/SDT
# generation. The zynq_ultra_ps_e CONFIG must match the base overlay's PS so the
# boot handoff (psu_init + device tree) presents the same PS to the OS: PL fabric
# clocks, PS<->PL AXI master/slave ports, and fabric reset.

set overlay_name "bsp"
set design_name "bsp"

set_param board.repoPaths board_files/rfsoc4x2/1.0/

create_project ${overlay_name} ${overlay_name} -part xczu48dr-ffvg1517-2-e -force
...
\
    ...
    CONFIG.PSU__CRL_APB__SPI1_REF_CTRL__ACT_FREQMHZ {19.999996} \
...
# Feed every PS clock input from pl_clk0 so validate_bd_design passes with no PL
# fabric present. These aclk nets do not affect the PS handoff (psu_init/DT),
# which is driven entirely by the CONFIG.PSU__* properties above.
set _ps [get_bd_cells zynq_ultra_ps_e_0]
set _plclk [get_bd_pins $_ps/pl_clk0]
foreach _p [get_bd_pins -of_objects $_ps -filter {DIR == I && TYPE == clk}] {
    if {[llength [get_bd_nets -quiet -of_objects $_p]] == 0} {
        connect_bd_net $_plclk $_p
    }
}
...
write_hw_platform -fixed -force ./${overlay_name}.xsa
